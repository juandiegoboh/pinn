...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct  7 10:22:36 2022
Ajuste de la red PINN en el contexto sismologico 04/Jun/2023
@author: sergio
"""
import pickle
...
nz=100# number of nodes along z axis.

n_abs=10# number of nodes for absorbing B.C in both directions, left and bottom
n_absx=n_abs# nodes from left side of the domain
n_absz=n_abs# nodes from bottom side of the domain

ax_spec=1.5# domain size in specfem 
az_spec=0.5# before removing absorbing regions
xsf=1.3# x location of all the seismometers in specfem

dx=ax_spec/nx# spatial step in x direction
dz=az_spec/nz# spatial step in z direction
rho=1.0# density

# we just need to remove the thickness of the absorbing B.C on the left and bottom side
ax=xsf-n_absx*dx# dimension of the domain in the x direction for PINNs training.
az=az_spec-n_absz*dz# dimension of the domain in the z direction for PINNs training.
...
t02=2300*s_spec# sec "initial" disp. input at this time from spec 
t_la=5000*s_spec# test data for comparing specfem and trained PINNs

n_event=1# number of seismic events
n_seis=20# number of input seismometers from SPECFEM;
# if events have different numbers of seismometers,
# you have to change the lines containing n_seis accordingly

z0_s=az# z location of the first seismometer from SPECFEM in PINN's refrence frame.
# Here it must be in km while in SPECFEM it's in meters. Note here we assume seismometers are
...
# to be zero and can be higher especially if you have absorbing B.C at the bottom,
# change this accordingly based on what you used from specfem 
...
# They define an ellipse with numerical values between [3, 2.5]
# The center is (0.85, 0.25) Km
# x-axis = 0.18 Km; z-axis = 0.1 Km
alpha_true=3-0.25*(1+tf.tanh(100*(1-g(x*Lx,z*Lz,0.18,0.1,1.0-n_absx*dx,0.3-n_absz*dz))))

# Define domain dimension for training both networks
ub=np.array([ax/Lx,az/Lz,(t_m-t_st)]).reshape(-1,1).T
ub0=np.array([ax/Lx,az/Lz]).reshape(-1,1).T

# Define stddev from number of inputs and outputs, assuming normal sampling
# according to Xavier Glorot to improve the optimization problem
...
# Define neural network for forward modelling
# The input layer is normalized to be between [-1 1]
# The inner layers are feed forward with tanh activation function
# The output layer does not have activation function
...
# Define neural network for inverse problem
# The input layer is normalized to be between [-1 1]
# The inner layers are feed forward with tanh activation function
# The output layer does not have activation function
...
layers=[3]+[30]*3+[1] 
# initialize weights and biases for NN that approximate the scalar acoustic potential
L = len(layers)
...
# layers for the second NN to approximate the wavespeed
layers0=[2]+[20]*5+[1] 
# initialize weights and biases for NN that approximate the wavespeed
L0 = len(layers0)
...
# Estimation of wavespeed using initial weights and biases
...
#np.save('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/X_pde', X_pde)
#sys.exit()
X_pde = np.load('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/X_pde.npy')
X_pde[:,0] = X_pde[:,0] * ax/Lx
X_pde[:,1] = X_pde[:,1] * az/Lz
X_pde[:,2] = X_pde[:,2] * (t_m-t_st)

###initial conditions for all events
X0=np.loadtxt('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/event1/wavefields/wavefield_grid_for_dumps_000.txt')# coordinates on which the wavefield output is recorded on specfem. It's the same for all the runs with the same meshing system in specfem
...
wfs = sorted(os.listdir('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/event1/wavefields/.'))
U0 = [np.loadtxt('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/event1/wavefields/'+f) for f in wfs]
...
plt.colorbar()
plt.axis('scaled')
plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/Ini_total_disp_spec_sumEvents.png', dpi=400)
plt.show()
plt.close(fig)
...
plt.colorbar()
plt.axis('scaled')
plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/sec_wavefield_input_spec_sumEvents.png', dpi=400)
plt.show()
plt.close(fig)
...
plt.colorbar()
plt.axis('scaled')
plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/total_disp_spec_testData_sumEvents.png', dpi=400)
plt.show()
plt.close(fig)
...
import os
sms = sorted(os.listdir('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/event1/seismograms/.'))
smsz = [f for f in sms if f[-6]=='Z']#Z cmp seismos
seismo_listz = [np.loadtxt('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/event1/seismograms/'+f) for f in smsz]#Z cmp seismos
...
seismo_listx = [np.loadtxt('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/event1/seismograms/'+f) for f in smsx]#X cmp seismos
...
def compute_jacobian(weights, biases, prediccion):
    J_list =[]
    L = len(weights)    
    for i in range(L):
        J_w = jacobian(prediccion, weights[i])
        J_list.append(J_w)
 
    for i in range(L):
        J_b = jacobian(prediccion, biases[i])
        J_list.append(J_b)
    return J_list

# Compute the empirical NTK = J J^T
def compute_ntk(J1_list, D1, J2_list, D2):
    N = len(J1_list)
    
    Ker = tf.zeros((D1.shape[0],D2.shape[0]),dtype=tf.dtypes.float64)
    for k in range(N):
        J1 = tf.reshape(J1_list[k], shape=(D1.shape[0],-1))
        J2 = tf.reshape(J2_list[k], shape=(D2.shape[0],-1))
...
x_p_ntk_tf = tf.placeholder(tf.float64, shape=(Np, 1))
x_eq_ntk_tf = tf.placeholder(tf.float64, shape=(Neq, 1))
x_ux_ntk_tf = tf.placeholder(tf.float64, shape=(Nux, 1))
x_uz_ntk_tf = tf.placeholder(tf.float64, shape=(Nuz, 1))
...
ntk_uz_snap1_dict = {x: X_init1[0:Nuz,0:1], z: X_init1[0:Nuz,1:2], t: X_init1[0:Nuz,2:3]}
ntk_ux_snap2_dict = {x: X_init2[0:Nux,0:1], z: X_init2[0:Nux,1:2], t: X_init2[0:Nux,2:3]}
ntk_uz_snap2_dict = {x: X_init2[0:Nuz,0:1], z: X_init2[0:Nuz,1:2], t: X_init2[0:Nuz,2:3]}
ntk_ux_seism_dict = {x: X_S[0:Nux_seism,0:1], z: X_S[0:Nux_seism,1:2], t: X_S[0:Nux_seism,2:3]}
ntk_uz_seism_dict = {x: X_S[0:Nuz_seism,0:1], z: X_S[0:Nuz_seism,1:2], t: X_S[0:Nuz_seism,2:3]}
...
plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/True_wavespeed.png', dpi=400)
plt.show()
plt.close(fig)
...
plt.colorbar()
plt.axis('scaled')
plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/Ini_guess_wavespeed.png', dpi=400)
plt.show()
plt.close(fig)
...
with tf.Session() as sess:
      ...
      for epoch in range(num_epoch):
          ...
          if epoch % 200 == 0:
              ...
              plt.colorbar()
              plt.axis('scaled')
              plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/Total_Predicted_dispfield_t='+str(0)+'.png',dpi=400)
              plt.show()
              plt.close(fig)
              fig = plt.figure()
              ...
              plt.colorbar()
              plt.axis('scaled')
              plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/Total_Predicted_dispfield_t='+str(round(t02-t01, 4))+'.png',dpi=400)
              plt.show()
              plt.close(fig)
              fig = plt.figure()
              ...
              plt.colorbar()
              plt.axis('scaled')
              plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/Total_Predicted_dispfield_t='+str(round((t_la-t01), 4))+'.png',dpi=400)
              plt.show()
              plt.close(fig)
              ...
              plt.colorbar()
              plt.axis('scaled')
              plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/pointwise_Error_spec_minus_PINNs_t='+str(round((t_la-t01), 4))+'.png',dpi=400)
              plt.show()
              plt.close(fig)
              ...
              plt.colorbar()
              plt.axis('scaled')
              plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/inverted_alpha.png',dpi=400)
              plt.show()
              plt.close(fig)
              ...
              plt.colorbar()
              plt.axis('scaled')
              plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/alpha_misfit.png',dpi=400)
              plt.show()
              plt.close(fig)
              ...
              plt.ylabel('misfit')
              plt.legend()
              plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/misfit.png',dpi=400)
              plt.show()
              plt.close(fig)
              ...
              plt.ylabel('Lambdas')
              plt.legend()
              plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/lambdas.png',dpi=400)
              plt.show()
              plt.close(fig)
              ...
              plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/ZSeismograms_compare_z='+str(round(az-d_s, 4))+'.png',dpi=400)
              plt.show()
              plt.close(fig)
              ...
              plt.savefig('/home/sergio/MEGA/PINN-FWI/rasht-behesht_etal_2021/XSeismograms_compare_z='+str(round(az-d_s, 4))+'.png',dpi=400)
              plt.show()
              plt.close(fig)
              
              
              print ('lam_p_val: ', lam_p_val, ', lam_eq_val: ', lam_eq_val, ', lam_disp1_val: ', lam_disp1_val, ', lam_disp2_val: ', lam_disp2_val, ', lam_seism_val: ', lam_seism_val)
