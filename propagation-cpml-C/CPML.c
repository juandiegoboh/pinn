                     ...
                     real vp, real fq)
{
  ...
  for (i = 0; i < nx; i++)
  {
    ...
    cpml->bx[i] = Exp(-(cpml->dcpm_x[i] / cpml->kappa_x[i] + cpml->alpha_x[i]) * dt);

    cpml->bx_half[i] = Exp(-(cpml->dcpm_x_half[i] / cpml->kappa_x_half[i] + cpml->alpha_x_half[i]) * dt);

    if (abs(cpml->dcpm_x[i]) > 1e-6)
    {
      cpml->ax[i] = cpml->dcpm_x[i] / (cpml->kappa_x[i] * (cpml->dcpm_x[i] + cpml->kappa_x[i] * cpml->alpha_x[i])) * (cpml->bx[i] - 1);
    }

    if (abs(cpml->dcpm_x_half[i]) > 1e-6)
    {
      cpml->ax_half[i] = cpml->dcpm_x_half[i] / (cpml->kappa_x_half[i] * (cpml->dcpm_x_half[i] + cpml->kappa_x_half[i] * cpml->alpha_x_half[i])) * (cpml->bx_half[i] - 1);
    }
  }
  ...
  for (i = 0; i < nz; i++)
  {
    ...
    cpml->bz[i] = Exp(-(cpml->dcpm_z[i] / cpml->kappa_z[i] + cpml->alpha_z[i]) * dt);

    cpml->bz_half[i] = Exp(-(cpml->dcpm_z_half[i] / cpml->kappa_z_half[i] + cpml->alpha_z_half[i]) * dt);

    if (abs(cpml->dcpm_z[i]) > 1e-6)
    {
      cpml->az[i] = cpml->dcpm_z[i] / (cpml->kappa_z[i] * (cpml->dcpm_z[i] + cpml->kappa_z[i] * cpml->alpha_z[i])) * (cpml->bz[i] - 1);
    }

    if (abs(cpml->dcpm_z_half[i]) > 1e-6)
    {
      cpml->az_half[i] = cpml->dcpm_z_half[i] / (cpml->kappa_z_half[i] * (cpml->dcpm_z_half[i] + cpml->kappa_z_half[i] * cpml->alpha_z_half[i])) * (cpml->bz_half[i] - 1);
    }
  }
}
                            ...
                            real cpml_thickness, real d0, real alpha_max_PML)
{
  ...
  if (abcissa_in_cpml > 0)
  {
    ...
    alpha[i] = (1 - abcissa_normalized) * alpha_max_PML + 0.1 * alpha_max_PML;
  }
}
